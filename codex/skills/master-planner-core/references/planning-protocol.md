...
This reference is the canonical planning workflow contract for the Codex master planner skill.
...
- strict enough that downstream execution skills do not guess
...
- produces or updates the next planning artifact
- enforces required user checkpoints
...
- return work to review and test skills outside the planning workflow
...
- exploratory spikes not intended to ship
- infrastructure work validated through non-test evidence only
...
After that:
- execution skills implement the approved tasks
- review and test skills validate implementation against the task packet and linked `TC-*` cases
